...
// MobileDevice can invoke the notification callback on a dispatch worker after
// the subscription call has returned. Keep the requested UDID in immutable C
// storage rather than comparing against a CFStringRef whose lifetime crosses
// that asynchronous boundary.
static char TargetIdentifierCString[256];
static BOOL TargetIdentifierReady;
...
static void DeviceCallback(AMDeviceNotificationCallbackInfo *info,
    ...
    char identifierCString[sizeof(TargetIdentifierCString)] = {0};
    BOOL matches = identifier && TargetIdentifierReady &&
        CFStringGetCString(identifier,
                           identifierCString,
                           sizeof(identifierCString),
                           kCFStringEncodingUTF8) &&
        strcmp(identifierCString, TargetIdentifierCString) == 0;
...
static BOOL SetTargetIdentifier(const char *value) {
    if (!value || !value[0] || strlen(value) >= sizeof(TargetIdentifierCString))
        return NO;
    strlcpy(TargetIdentifierCString,
            value,
            sizeof(TargetIdentifierCString));
    TargetIdentifierReady = YES;
    return YES;
}
...
static NSDictionary *FinishWrite(DeviceSession *session, NSArray<NSString *> *args) {
    ...
    BOOL safeArguments = GeneratedNamesMatch(source, linkDestination, recovered) &&
        LoadBooksSnapshot(snapshotRoot) != nil;
    if (!safeArguments)
        return @{ @"ok": @NO, @"safeArguments": @NO };
...
static NSDictionary *FinishMovedRemoval(DeviceSession *session,
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if ([command isEqual:@"syslog"] && argc == 3) {
            if (!SetTargetIdentifier(argv[2])) return 64;
            int status = RunSyslog();
            if (TargetDevice) {
                CFRelease(TargetDevice);
                TargetDevice = NULL;
            }
            TargetIdentifierReady = NO;
            return status;
        }
